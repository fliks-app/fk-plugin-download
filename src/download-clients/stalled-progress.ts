/**
 * Ported from `backend/src/plugins/download/stalled-progress.util.ts`. That
 * version compares bigint-as-string byte counters; this plugin's
 * `stalled_checks.downloadedBytes` already round-trips as a lossless JS
 * `number` (see `src/db/pool.ts`'s OID-20 type parser), so the tolerance
...
/**
 * qBittorrent raw states eligible for stall detection — states where
 * download progress is expected, so a flat byte counter means the torrent
 * is stuck. Everything else (paused/stopped/queued/checking/allocating/moving
 * and every seeding-side state) makes no progress by design and must never
 * be flagged. `error` and `missingFiles` go through the same stall window as
 ...
 * newest-first (DESC by `checkedAt`) — the order the caller queries in.
 *
