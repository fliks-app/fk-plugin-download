...
How this plugin is built and how to work on it. For what it does and how to set it up, see
[README.md](../README.md).
...
The catalogue owns signing: commit this plugin's built `plugin.json`, `plugin.js` and `logo.svg`
under `plugins/fliks.download/src/` in `fliks-app/fliks-plugin-catalog`, add a
`versions/<version>.json` entry, then run its **Package plugin** workflow for this id — it signs
with the catalogue key and records the published archive's own sha256 on the entry.

Two things that bite: a published version's bytes are pinned by that checksum, so any change
ships as a new version rather than as a correction to the old one; and the packaging workflow's
concurrency is per plugin id, so two ids dispatched at once both push and one loses the ref race.
